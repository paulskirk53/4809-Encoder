...
void aggregateASCOMCommand(USART_t* beans, char* command)
{
  char c;
  uint8_t index = 0;
  while (c != '~') // A RANDOOM UNUSED CHAR TO FACILITATE THE LOOP
  {
    if ((beans->STATUS & USART_RXCIF_bm)) //if there's a char available...

    {

      c = USARTx_readChar(beans);            // read the char  pass *beans ???
      if (c != '\n' && c != '\r' && c != '#') // # marks end of command string
      {

        if (validchar(c)) //Valid strings of chars are: AZ# (caps) and STA followed by 1 to 6 chars e.g. 1.00# , 342.00#
        {
